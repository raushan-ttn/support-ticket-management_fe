...
1. **Classify** (symptom → first place to look):
   ...
   - 401 every request → httpOnly cookie missing / `setAuthCookieAction` not called after login
...
2. **Trace** — server path (terminal first) vs client path (Redux DevTools → server terminal for `tmsFetch`) vs auth path (cookie → `tmsFetch` → backend).
